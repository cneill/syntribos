...
class HTTPSignal(syntribos.signal):
    ...
    def __init__(self, *args, **kwargs):
        super(HTTPSignal, self).__init__(*args, **kwargs)
        ...
        if self.response:
            ...
            self.status_reason = self.response.reason
...
class HTTPFailureSignal(HTTPSignal):
    ...
    def from_requests_exception(cls, exception):
        if not cls.is_covered_exception(exception):
            return None
        ...
        slug = "{type}_{exception}".format(
            cls.signal_type, exception.__name__.upper())

        return cls(
            status_code=status_code, text=text, request=request,
            response=response, slug=slug, tags=tags)
